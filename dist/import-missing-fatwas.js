...
async function main() {
    ...
    console.log('\nAvailable sources:');
    sources.forEach((s) => console.log(`  ${s.id} | ${s.name} | ${s.slug}`));
    const category = categories.find((c) => c.name.includes('عبادة') ||
        c.name.includes('فقه') ||
        c.name.includes('آداب') ||
        c.slug.includes('ibada') ||
        c.slug.includes('fiqh') ||
        c.slug.includes('adab')) || categories[0];
    console.log(`\nSelected category: ${category.name} (${category.id})`);
    const binbazSource = sources.find((s) => s.slug.includes('binbaz') || s.name.includes('باز')) || sources[0];
    const uthaymeenSource = sources.find((s) => s.slug.includes('uthaymeen') || s.name.includes('عثيمين')) || sources[0];
    const fawzanSource = sources.find((s) => s.slug.includes('fawzan') || s.name.includes('فوزان')) || sources[0];
    console.log(`\nSources selected:`);
...
//# sourceMappingURL=import-missing-fatwas.js.map