...
import { coalesceAsync, isObject, lessThan, runIfFn } from "@cacheable/utils";
import { Keyv } from "keyv";
//#region src/keyv-adapter.ts
var KeyvAdapter = class {
	...
	async has(key) {
		if (await this._cache.get(key)) return true;
		return false;
	}
	async getMany(keys) {
...
//#endregion
//#region src/index.ts
const _storeLabel = (i) => i === 0 ? "primary" : `secondary:${i - 1}`;
const createCache = (options) => {
	...
	const mget = async (keys) => {
		...
		if (nonBlocking) try {
			...
			eventEmitter.emit("mget", {
				keys,
				error
			});
		}
		else for (const store of stores) try {
			const missingValues = result.map((value, index) => value === void 0 ? {
				originalIndex: index,
				key: keys[index]
	...
	const ttl = async (key) => {
		...
		if (result?.expires) return result.expires;
	};
	const set = async (stores, key, value, ttl) => {
		try {
	...
	const mset = async (stores, rawList) => {
	...
	const mdel = async (keys) => {
		try {
			const promises = [];
			for (const key of keys) promises.push(...stores.map(async (store) => store.delete(key)));
			if (nonBlocking) {
				Promise.all(promises);
	...
	const wrap = async (key, fnc, ttlOrOptions, refreshThresholdParameter) => coalesceAsync(`${_cacheId}::${key}`, async () => {
		...
		let i = 0;
		let remainingTtl;
		const { ttl, refreshThreshold, raw } = isObject(ttlOrOptions) ? ttlOrOptions : {
			ttl: ttlOrOptions,
			refreshThreshold: refreshThresholdParameter
		};
		const resolveTtl = (result) => runIfFn(ttl, result) ?? options?.ttl;
		for (; i < stores.length; i++) try {
			const data = await stores[i].get(key, { raw: true });
			if (data !== void 0) {
				value = data.value;
				rawData = data;
				if (typeof data.expires === "number") remainingTtl = Math.max(0, data.expires - Date.now());
				break;
			}
		} catch {}
		if (value === void 0) {
		...
		const shouldRefresh = lessThan(remainingTtl, runIfFn(refreshThreshold, value) ?? options?.refreshThreshold);
		if (shouldRefresh) coalesceAsync(`+++${_cacheId}__${key}`, fnc).then(async (result) => {
			try {
		...
		if (!shouldRefresh && i > 0) await set(stores.slice(0, i), key, value, resolveTtl(value));
...
//#endregion
export { KeyvAdapter, createCache };
