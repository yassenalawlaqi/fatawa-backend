...
import { ContentExtractorService, ExtractionStrategy } from './content-extractor.service';
...
describe('ContentExtractorService', () => {
  ...
  describe('extractContent', () => {
    it('should return data from the first successful strategy', async () => {
      jest.spyOn(service['axiosClient'], 'get').mockResolvedValue({ data: 'API Data' } as any);
    ...
    it('should fall back to the next strategy if the first fails', async () => {
      jest.spyOn(service['axiosClient'], 'get')
        ...
        .mockResolvedValueOnce({ data: 'HTML Data' } as any);
  ...
  describe('extractAttachments', () => {
    it('should extract PDF, audio, and video links', () => {
      ...
      expect(attachments[1]).toEqual({
        type: 'audio',
        url: 'https://example.com/audio.mp3', // Absolute URL remains unchanged
        title: 'ملف صوتي'
      });
