...
describe('SearchRepository', () => {
  ...
  describe('search', () => {
    it('should use FTS if it returns results', async () => {
      jest.spyOn(repository, 'searchFTS').mockResolvedValue({ data: [{ id: '1' } as any], total: 1 });
    ...
    it('should fallback if FTS returns 0 results', async () => {
      ...
      jest.spyOn(repository, 'searchFallback').mockResolvedValue({ data: [{ id: '1' } as any], total: 1 });
  ...
  describe('searchFTS', () => {
    it('should call $queryRaw for FTS search', async () => {
      prismaService.$queryRaw.mockImplementation(async (query: any) => {
        if (query.strings[0].includes('COUNT')) return [{ total: 1 }];
  ...
  describe('searchFallback', () => {
    ...
    it('should execute Prisma query and manually score the results', async () => {
      ...
      prismaService.fatwa.findMany.mockResolvedValue(mockFatawa as any);

      const result = await repository.searchFallback('الصلاة', 1, 10);
      
      expect(prismaService.fatwa.findMany).toHaveBeenCalled();
      expect(result.total).toBe(2);
      expect(result.data[0].id).toBe('1'); // ID 1 gets higher score because 'الصلاة' is in category
      expect(result.data[0].score).toBeGreaterThan(result.data[1].score as number);
  ...
  describe('logSearch', () => {
    it('should create log', async () => {
      prismaService.searchLog.create.mockResolvedValue({} as any);
  ...
  describe('getTrendingSearches', () => {
    it('should fetch grouped queries', async () => {
      prismaService.searchLog.groupBy.mockResolvedValue([{ query: 'الصلاة' } as any]);
